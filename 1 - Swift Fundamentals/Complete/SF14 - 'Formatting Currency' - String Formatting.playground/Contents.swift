...
func USDCalculation(amount: Double) -> String {
    let inDollars = amount.formatted(.currency(code: "USD").presentation(.narrow))
    return inDollars
}

print(USDCalculation(amount: 3))

func JPYCalculation(USDamount: Double) -> String {
    let inYen = USDamount * 1.47
    let yenFormatted = inYen.formatted(.currency(code: "JPY").presentation(.narrow))
    return yenFormatted
}

print(JPYCalculation(USDamount: 3))
